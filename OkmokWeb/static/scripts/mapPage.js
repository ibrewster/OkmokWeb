...
function generateGraphs() {
    ...
    var sensor = optionsDiv.find('input[type=radio]:checked').data('sensor');
...
function createChartHeader(station, site) {
    ...
    for (var idx in channelOptions) {
        ...
        button.data('sensor', channelVals[idx]);
...
function graphResults(respData, dest) {
    ...
    var sensor = channelOpt.data('sensor');
    ...
    if (sensor != 'CO2') {
        ...
        const filtered = makePlotDataDict(data['dates'], data['co2filtered'])
